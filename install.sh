  ...
  custom_mounts=$(jq -c '
    .mounts // [] | map(
      select(
        ...
        (contains("target=/home/vscode/.config/zsh,") | not) and
        (contains("target=/home/vscode/.oh-my-zsh/custom,") | not) and
...
cmd_update() {
  ...
  if ! git -C "$SCRIPT_DIR" rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    log_info "Re-clone with: rm -rf ~/.agent-devcontainer && git clone https://github.com/MadhavM-117/agent-devcontainer ~/.agent-devcontainer"
    exit 1
  fi
